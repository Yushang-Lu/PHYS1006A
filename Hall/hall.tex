...
\subsubsection{由表1中的数据可以画出$V_H-I_M$曲线，如图1所示：}
...
\subsubsection{由表2中的数据可以画出$V_H-I_S$曲线，如图2所示：}
...
\subsubsection{由 $V_H = K_H I_S B$ 计算霍尔元件的灵敏度$K_H$：}
...
$$ \overline{K}_H= \frac{1}{2}\left(178.195 + 178.431\right) = 178.313 \ \mathrm{m^2/C} $$
...
由图5可见，输出电压$V_{OUT}$随转角$\theta$的增大而单调增大，且变化规律呈明显非线性，$V_{OUT}-\theta$曲线近似余弦曲线，符合规律 $R = R_{min}+\left(R_{max}-R_{min}\right)\cos^2\theta$。

\section{讨论问题}

\subsection{如何根据$B$、$I_H$和$U_H$方向判断霍尔片的导电类型（N型或P型半导体），要求画图说明。（注：N型半导体中，载流子为电子；P型半导体中将载流子视为正离子）；}

\begin{center}
    \includegraphics[width=0.5\textwidth]{hall_element.jpg}
    
    图6 霍尔元件示意图
\end{center}

如图为霍尔元件的示意图。坐标轴规定了电流$I_H$、磁场$B$、电压$U_H$的正方向。

假设霍尔片是$N$型半导体，载流子为电子。电子的流动方向与电流方向相反，由于洛伦兹力的作用，电子向图中的$f_L$方向偏转，造成$b$侧负电荷的积累，最终在$a$，$b$面建立霍尔电场$E_H$，以及电势差$U_H$。

因此，通过测量霍尔电压$U_H$的极性，可以判断霍尔片的导电类型：
\begin{itemize}
    \item 如果$U_H$为正电压，则霍尔片为N型半导体。
    \item 如果$U_H$为负电压，则霍尔片为P型半导体。
\end{itemize}

\subsection{估算本实验所用霍尔片的载流子浓度。}

已知本实验中所用霍尔片的厚度 $d = 1 mm$，由公式：

$$ K_H = \frac{1}{n e d} $$

其中 e 取 $1.602\times 10^{-19}C$，可得本实验所用霍尔片的载流子浓度为：

$$ n = \frac{1}{e K_H d} = 3.18 \times 10^{18} \ \mathrm{\left(m^{-3}\right)} $$
